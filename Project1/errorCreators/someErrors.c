...
int llwrite(int fd, unsigned char* buffer, int length){
	...
	unsigned char *frame, *stuffed, *badFrame, bytesStream[1];

	int frameLength, stuffedLength, rNumber;
	...
	badFrame = (unsigned char *) malloc(frameLength);
	...
	memcpy(badFrame, frame, frameLength);

	badFrame[getRandomNumber() % 30] = (unsigned char)getRandomNumber();
	...
	while(getMachineState(detectRR) != FINAL_STATE) {
		if(getFlag(linkL) == 0) {
			...
			if((rNumber = getRandomNumber()) <= 20) {
				if(write_frame(fd, badFrame, frameLength) == -1) {
